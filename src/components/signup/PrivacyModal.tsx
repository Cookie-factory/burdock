...
import CustomModalContent from '../common/modal/ModalContent';
import CustomActionSheet from '../common/modal/CustomActionSheet';
...
/**
 *@description 개인정보처리 방침 모달
 */
function PrivacyModal({isOpen, onClose}: Props) {
  return (
    <CustomActionSheet isOpen={isOpen} onClose={onClose}>
      <CustomModalContent minH={480} h={480} px={32} py={34}>
...
픽코믹스 (이하 ‘회사'라고 함)는 회사가 제공하고자 하는 서비스(이하 ‘회사 서비스’)를 이용하는 개인(이하 ‘이용자’ 또는 ‘개인’)의 정보(이하 ‘개인정보’)를 보호하기 위해, 개인정보보호법, 정보통신망 이용촉진 및 정보보호 등에 관한 법률(이하 '정보통신망법') 등 관련 법령을 준수하고, 서비스 이용자의 개인정보 보호 관련한 고충을 신속하고 원활하게 처리할 수 있도록 하기 위하여 다음과 같이 개인정보처리방침(이하 ‘본 방침’)을 수립합니다.
...
1. 회사는 이용자가 언제든지 쉽게 본 방침을 확인할 수 있도록 앱 설정의 연결화면을 통해 본 방침을 공개하고 있습니다.
...
2. 회사는 제1항에 따라 본 방침을 개정하는 경우 다음 각 호 하나 이상의 방법으로 공지합니다.
    ...
    가.  회사가 운영하는 앱 공지사항란 또는 별도의 창을 통하여 공지하는 방법
...
1. 필수 수집 정보: 이메일 주소, 비밀번호, 닉네임, 연령대, 성별
